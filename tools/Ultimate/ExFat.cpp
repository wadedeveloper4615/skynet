...
void CExFat::OnSelchangedDirTree(NMHDR* pNMHDR, LRESULT* pResult) {
  ...
  S_EXFAT_ITEMS *items = (S_EXFAT_ITEMS *) m_dir_tree.GetDataStruct(hItem);
...
void CExFat::Start(const DWORD64 lba, const DWORD64 size, const DWORD color, const int index, BOOL IsNewTab) {
  ...
  S_EXFAT_VBR *vbr = (S_EXFAT_VBR *) m_vbr_buffer;
  ...
  SendToDialog((S_EXFAT_VBR *) m_vbr_buffer);
  ...
  if (m_isvalid) {
    ...
    S_EXFAT_ROOT *root;
    DWORD64 rootsize = 0;
    root = (S_EXFAT_ROOT *) ReadFile(vbr->root_dir_cluster, &rootsize, 0);
    if (root) {
...
void CExFat::ParseDir(S_EXFAT_ROOT *root, DWORD64 root_size, HTREEITEM parent) {
  S_EXFAT_ROOT *cur = root;
    ...
    switch (cur->entry_type) {
      ...
      case EXFAT_DIR_ENTRY:  // Entry type: Directory Entry
        ...
        if (((secondary_count >= EXFAT_MIN_SECONDARY_COUNT) && (secondary_count <= EXFAT_MAX_SECONDARY_COUNT)) &&
            (ExFatCheckDirCRC((BYTE *) cur, ((secondary_count + 1) * sizeof(S_EXFAT_ROOT))) == cur->type.dir_entry.crc)) {
            ...
            if (cur->entry_type == EXFAT_DIR_DELETED_S) {
              ...
              if (cur->type.dir_entry.attributes & EXFAT_ATTR_SUB_DIR) {
                ...
                if (!IsDot) {
                  ...
                  S_EXFAT_ROOT *sub = (S_EXFAT_ROOT *) ReadFile(cluster, &size, flags);
                  if (sub) {
...
int CExFat::AllocateFAT(S_FAT_ENTRIES *EntryList, DWORD size, BYTE *flags) {
  S_EXFAT_VBR *vbr = (S_EXFAT_VBR *) m_vbr_buffer;
  const unsigned sect_per_clust = (1 << vbr->log_sects_per_clust);
  const unsigned bytes_per_sect = (1 << vbr->log_bytes_per_sect);
...
int CExFat::ExFatFillClusterList(S_FAT_ENTRIES *EntryList, DWORD StartCluster, BYTE Flags) {
  EntryList->was_error = FALSE;
  EntryList->entry_count = 0;
...
void CExFat::AllocateRoot(CString csName, S_EXFAT_ITEMS *RootItems, DWORD StartCluster, DWORD FileSize, BYTE flags, bool isFolder) {
  S_EXFAT_ROOT *entry;
  ...
  void *root = (S_EXFAT_ROOT *) ReadFile(RootItems->Cluster, &rootsize, RootItems->Flags);
  if (root) {
    entry = (S_EXFAT_ROOT *) root;
    ...
    entry[0].type.dir_entry.crc = ExFatCheckDirCRC((BYTE *) &entry[0], ((secondary_count + 1) * sizeof(S_EXFAT_ROOT)));
...
void CExFat::SendToDialog(S_EXFAT_VBR *vbr) {
...
void CExFat::ReceiveFromDialog(S_EXFAT_VBR *vbr) {
...
void CExFat::OnExFatEntry() {
  HTREEITEM hItem = m_dir_tree.GetSelectedItem();
  S_EXFAT_ITEMS *items, *parent_items = NULL;
  ...
  if (hItem) {
    ...
    items = (S_EXFAT_ITEMS *) m_dir_tree.GetDataStruct(hItem);
    if (items) {
      // if (parent_items == NULL) we will need to calcute the root cluster number from the vbr
      if (items->hParent) {
        parent_items = (S_EXFAT_ITEMS *) m_dir_tree.GetDataStruct(items->hParent);
...
void CExFat::OnFysosSig() {
  ...
  memcpy((BYTE *) m_vbr_buffer + S_FYSOSSIG_OFFSET, buffer + S_FYSOSSIG_OFFSET, sizeof(S_FYSOSSIG));
  OnExFatApply();
}

void CExFat::OnExFatApply() {
  CUltimateDlg *dlg = (CUltimateDlg *) AfxGetApp()->m_pMainWnd;
  ReceiveFromDialog((S_EXFAT_VBR *) m_vbr_buffer);
...
bool CExFat::ExFatFormat(const BOOL AskForBoot) {
  ...
  S_EXFAT_VBR *vbr = (S_EXFAT_VBR *) buffer;
  if (!m_hard_format)
    memcpy((BYTE *) vbr + off, (BYTE *) m_vbr_buffer + off, sizeof(S_EXFAT_VBR) - off);
  ...
  *w = 0xAA55;
  ...
  // (3 * 32-bytes each < 512)
  memset(sect_buff, 0, dlg->m_sect_size);
  S_EXFAT_ROOT *root = (S_EXFAT_ROOT *) sect_buff;
  // label entry
  ...
  if (!m_hard_format) {
    memcpy(m_vbr_buffer, vbr, sizeof(S_EXFAT_VBR));
    SendToDialog((S_EXFAT_VBR *) m_vbr_buffer);
  }
...
void CExFat::OnUpdateCode() {
  ...
  if (odlg.DoModal() == IDOK) {
    ...
    memcpy(existing + sizeof(S_EXFAT_VBR), buffer + sizeof(S_EXFAT_VBR), (9 * dlg->m_sect_size) - sizeof(S_EXFAT_VBR));
    ...
    *w = 0xAA55;
    ...
    memcpy((BYTE *) m_vbr_buffer + sizeof(S_EXFAT_VBR), existing + sizeof(S_EXFAT_VBR), (12 * dlg->m_sect_size) - sizeof(S_EXFAT_VBR));
...
void CExFat::InsertFolder(S_EXFAT_ITEMS *RootItems, CString csName, CString csPath) {
  S_EXFAT_VBR *vbr = (S_EXFAT_VBR *) m_vbr_buffer;
  const unsigned sect_per_clust = (1 << vbr->log_sects_per_clust);
  const unsigned bytes_per_sect = (1 << vbr->log_bytes_per_sect);
...
BOOL CExFat::InsertFile(S_EXFAT_ITEMS *RootItems, CString csName, CString csPath) {
  S_EXFAT_VBR *vbr = (S_EXFAT_VBR *) m_vbr_buffer;
  const unsigned sect_per_clust = (1 << vbr->log_sects_per_clust);
  const unsigned bytes_per_sect = (1 << vbr->log_bytes_per_sect);
  S_FAT_ENTRIES fat_entries;
...
void CExFat::DeleteFile(HTREEITEM hItem) {
  ...
  S_EXFAT_ITEMS *root_items = (S_EXFAT_ITEMS *) m_dir_tree.GetDataStruct(hParent);
  if (root_items == NULL)
    return;
  ...
  if (root) {
    S_EXFAT_ROOT *entry = (S_EXFAT_ROOT *) ((BYTE *) root + items->EntryOffset);
    int secondary_count;
    switch (entry->entry_type) {
      ...
      case EXFAT_DIR_ENTRY:  // Entry type: Directory Entry
        ...
        // update the CRC
        entry->type.dir_entry.crc = ExFatCheckDirCRC((BYTE *) entry, ((secondary_count + 1) * sizeof(S_EXFAT_ROOT)));
