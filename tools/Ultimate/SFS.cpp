...
void CSFS::OnSFSApply() {
  ...
  memcpy(buffer + SFS_SUPER_LOC, &m_super, sizeof(S_SFS_SUPER));
...
void CSFS::OnUpdateCode() {
  ...
  if (odlg.DoModal() == IDOK) {
    ...
    memcpy(existing + SFS_SUPER_LOC + sizeof(S_SFS_SUPER), buffer + SFS_SUPER_LOC + sizeof(S_SFS_SUPER), reserved - (SFS_SUPER_LOC + sizeof(S_SFS_SUPER)));
...
void CSFS::Start(const DWORD64 lba, const DWORD64 size, const DWORD color, const int index, BOOL IsNewTab) {
  ...
  memcpy(&m_super, buffer + SFS_SUPER_LOC, sizeof(S_SFS_SUPER));
...
void CSFS::SendToDialog(S_SFS_SUPER *super) {
  CString cs;
...
void CSFS::ReceiveFromDialog(S_SFS_SUPER *super) {
