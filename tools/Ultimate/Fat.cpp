...
void CFat::OnSelchangedDirTree(NMHDR* pNMHDR, LRESULT* pResult) {
  ...
  S_FAT_ITEMS *items = (S_FAT_ITEMS *) m_dir_tree.GetDataStruct(hItem);
...
void CFat::OnFatApply() {
  ...
  if (m_fat_size == FS_FAT32)
    memcpy(buffer, m_bpb_buffer, sizeof(S_FAT32_BPB));
  else
    memcpy(buffer, m_bpb_buffer, sizeof(S_FAT1216_BPB));
...
bool CFat::FatFormat(const BOOL AskForBoot) {
  ...
  if (m_fat_size == FS_FAT32) {
    S_FAT32_BPB *bpb = (S_FAT32_BPB *) buffer;
    if (!has_boot_sector)
    ...
    S_FAT1216_BPB *bpb = (S_FAT1216_BPB *) buffer;
  ...
  if (m_fat_size == FS_FAT32) {
    S_FAT32_BPB_INFO *info = (S_FAT32_BPB_INFO *) buffer;
...
void CFat::OnUpdateCode() {
  CUltimateDlg *dlg = (CUltimateDlg *) AfxGetApp()->m_pMainWnd;
  S_FAT32_BPB *bpb32 = (S_FAT32_BPB *) m_bpb_buffer;
  S_FAT1216_BPB *bpb12 = (S_FAT1216_BPB *) m_bpb_buffer;
  S_FYSOSSIG s_sig;
  CFile bsfile;
  CString cs;
  unsigned extra = 0;
  ...
  if (odlg.DoModal() == IDOK) {
    ...
    if (m_fat_size == FS_FAT32) {
      ...
      memcpy(existing + sizeof(S_FAT32_BPB), buffer + sizeof(S_FAT32_BPB), dlg->m_sect_size - sizeof(S_FAT32_BPB));
      ...
      memcpy(existing + sizeof(S_FAT1216_BPB), buffer + sizeof(S_FAT1216_BPB), (reserved * dlg->m_sect_size) - sizeof(S_FAT1216_BPB));
    ...
    if (extra > 0) {
      ((S_FAT1216_BPB *) existing)->sect_reserved = extra;
  ...
  if (m_isvalid) {
    ...
    if (m_fat_size == FS_FAT32) {
      ...
      if (rootcluster > 0)  // make sure there is actually a root to read
        root = (S_FAT_ROOT *) ReadFile(rootcluster, &m_rootsize, TRUE);
    } else {
      rootcluster = 0;
      m_rootsize = bpb12->root_entrys * sizeof(S_FAT_ROOT);
    ...
    if (root) {
      ...
      ParseDir(root, m_rootsize / sizeof(S_FAT_ROOT), m_hRoot, TRUE);
...
bool CFat::ParseDir(S_FAT_ROOT *root, const unsigned entries, HTREEITEM parent, BOOL IsRoot) {
  S_FAT_ROOT *sub;
  HTREEITEM hItem;
  unsigned cnt, i = 0;
  CString name;
    ...
    if (ErrorCode == FAT_BAD_LFN_DEL)
      ...
      if (root[i].name[0] == FAT_DELETED_CHAR) {
        ...
        if (attrb & FAT_ATTR_SUB_DIR) {
          ...
          if (!IsDot) {
            sub = (S_FAT_ROOT *) ReadFile(start, &filesize, FALSE);
            if (sub) {
...
void *CFat::ReadFile(DWORD cluster, DWORD *size, BOOL IsRoot) {
  CUltimateDlg *dlg = (CUltimateDlg *) AfxGetApp()->m_pMainWnd;
  S_FAT1216_BPB *bpb = (S_FAT1216_BPB *) m_bpb_buffer;
  void *ptr = NULL;
...
void CFat::WriteFile(void *buffer, S_FAT_ENTRIES *ClusterList, DWORD size, BOOL IsRoot) {
  CUltimateDlg *dlg = (CUltimateDlg *) AfxGetApp()->m_pMainWnd;
  S_FAT1216_BPB *bpb = (S_FAT1216_BPB *) m_bpb_buffer;
  unsigned pos = 0;
...
// checks a ROOT entry
DWORD CFat::CheckRootEntry(S_FAT_ROOT *root) {
  unsigned i, sfn = 0;
  ...
  if (root->attrb == FAT_ATTR_LONG_FILE) {
    DWORD ErrorCode;
    sfn = FatCheckLFN((S_FAT_LFN_ROOT *) root, &ErrorCode);
...
unsigned CFat::FatGetName(S_FAT_ROOT *root, CString &name, BYTE *attrb, DWORD *start, DWORD *filesize, BOOL *IsDot) {
  unsigned i = 0;
  CString ext;
  
  if (root->attrb == FAT_ATTR_LONG_FILE)
    i = FatGetLFN((S_FAT_LFN_ROOT *) root, name);
  else {
...
unsigned CFat::FatGetLFN(S_FAT_LFN_ROOT *lfn, CString &name) {
...
unsigned CFat::FatCheckLFN(S_FAT_LFN_ROOT *lfn, DWORD *ErrorCode) {
  int i, j, cnt = lfn->sequ_flags & ~0xC0;
  char *t, str[1024];
  ...
  S_FAT_ROOT *root = (S_FAT_ROOT *) ((BYTE *) lfn + (cnt * 32));
...
int CFat::FatFillClusterList(S_FAT_ENTRIES *EntryList, DWORD Cluster) {
  CWaitCursor wait; // display a wait cursor
...
int CFat::AllocateFAT(S_FAT_ENTRIES *EntryList, DWORD size) {
  S_FAT1216_BPB *bpb = (S_FAT1216_BPB *) m_bpb_buffer;
...
void CFat::AllocateRoot(CString csName, DWORD RootCluster, DWORD StartCluster, DWORD FileSize, BYTE Attribute, BOOL IsRoot) {
  ...
  root = (S_FAT_ROOT *) ReadFile(RootCluster, &rootsize, IsRoot);
  if (root) {
    r = root;
    i = 0;
    lfn = NULL;
...
BYTE CFat::CalcCRCFromSFN(S_FAT_ROOT *entry) {
...
void CFat::SendToDialog(void *ptr) {
  ...
  if (m_fat_size == FS_FAT32) {
    S_FAT32_BPB *bpb = (S_FAT32_BPB *) ptr;
    ...
    S_FAT1216_BPB *bpb = (S_FAT1216_BPB *) ptr;
    m_jmp0.Format("0x%02X", bpb->jmp[0]);
    m_jmp1.Format("0x%02X", bpb->jmp[1]);
    m_jmp2.Format("0x%02X", bpb->jmp[2]);
    m_oem_name.Format("%c%c%c%c%c%c%c%c", bpb->oem_name[0], bpb->oem_name[1], bpb->oem_name[2], bpb->oem_name[3],
...
void CFat::SaveItemInfo(HTREEITEM hItem, DWORD Cluster, DWORD FileSize, S_FAT_ROOT *Entry, int Index, int count, DWORD ErrorCode, BOOL CanCopy) {
  S_FAT_ITEMS *items = (S_FAT_ITEMS *) m_dir_tree.GetDataStruct(hItem);
  if (items) {
    ...
    if (Entry)
      memcpy(items->Entry, Entry, (count * sizeof(S_FAT_ROOT)));
  }
}
...
void CFat::DeleteFile(HTREEITEM hItem) {
  ...
  S_FAT_ITEMS *root_items = (S_FAT_ITEMS *) m_dir_tree.GetDataStruct(hParent);
  if (root_items == NULL)
    return;
  
  m_dir_tree.GetFullPath(hParent, NULL, csName, csPath, TRUE);
  DWORD rootsize = (IsRoot) ? m_rootsize : 0, cluster;
  root = (S_FAT_ROOT *) ReadFile(root_items->Cluster, &rootsize, IsRoot);
  if (root) {
...
void CFat::UpdateEntry(HTREEITEM hItem, S_FAT_ITEMS *entry_items) {
  S_FAT_ROOT *root;
  CString csName, csPath;
  int i;
  ...
  if (root) {
    for (i=0; i<entry_items->EntryCount; i++)
      memcpy(&root[entry_items->Index + i], &entry_items->Entry[i], sizeof(S_FAT_ROOT));
...
void CFat::OnFatEntry() {
  ...
  if (hItem) {
    ...
    if (items) {
      ...
      FatEntry.m_lfns = malloc(ROOT_ENTRY_MAX * sizeof(S_FAT_ROOT));
      memcpy(FatEntry.m_lfns, &items->Entry, ROOT_ENTRY_MAX * sizeof(S_FAT_ROOT));
      FatEntry.m_lfn_count = items->EntryCount - 1;
      FatEntry.m_fat_size = m_fat_size;
      memcpy(&FatEntry.m_sfn, &items->Entry[sfn_index], sizeof(S_FAT_ROOT));
      ...
      if (FatEntry.DoModal() == IDOK) { // apply button pressed?
        memcpy(&items->Entry, FatEntry.m_lfns, ROOT_ENTRY_MAX * sizeof(S_FAT_ROOT));
