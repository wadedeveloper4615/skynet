...
typedef struct _S_LEAN_SUPER {
  ...
  S_GUID guid;             // Globally Unique IDentifier
...
typedef struct _S_LEAN_INDIRECT1 {
...
typedef struct _S_LEAN_DIRENTRY1 {
...
}S_LEAN_DIRENTRY;                  // 4 to make it 16 bytes for first para.  Not limited to 4 bytes.
...
typedef struct _S_LEAN_JOURNAL1 {
...
typedef struct _S_LEAN_JOURNAL_ENTRY1 {
  DWORD64 new_inode;
  DWORD64 org_inode;
  DWORD   flags;       // valid entry, etc.
  DWORD   resv[3];     // for future expansion
}S_LEAN_JOURNAL_ENTRY;


///////////////////////////////////////////////////////////////////////////////////
// cannot be > 240 bytes
typedef struct _S_LEAN_ITEMS1 {
  DWORD64 Inode;
  DWORD64 FileSize;
  DWORD ErrorCode;
...
typedef struct _S_LEAN_BLOCKS1 {
  BOOL   was_error;
...
// Dialog Data
  ...
  void ParseDir(S_LEAN_DIRENTRY *root, DWORD64 root_size, HTREEITEM parent);
  void *ReadFile(DWORD64 block, DWORD64 *Size);
  void WriteFile(void *buffer, S_LEAN_BLOCKS *Extents, DWORD64 Size);
  void ZeroExtent(DWORD64 ExtentStart, DWORD ExtentSize);
  BOOL ValidInode(const S_LEAN_INODE *inode);
  BOOL ValidIndirect(const S_LEAN_INDIRECT *indirect);
  ...
  void SendToDialog(S_LEAN_SUPER *super);
  void ReceiveFromDialog(S_LEAN_SUPER *super);
  ...
  int AppendToExtents(S_LEAN_BLOCKS *Extents, DWORD64 Size, DWORD64 Start, BOOL MarkIt);
  void FreeExtents(const S_LEAN_BLOCKS *Extents);
  int ReadFileExtents(S_LEAN_BLOCKS *Extents, DWORD64 Inode);
  int WriteFileExtents(const S_LEAN_BLOCKS *Extents, S_LEAN_INODE *inode);
  int AllocateRoot(CString csName, DWORD64 Inode, DWORD64 Start, BYTE Attrib);
  void BuildInode(S_LEAN_BLOCKS *Extents, DWORD64 Size, DWORD Attrib);
  void IncrementLinkCount(DWORD64 Inode);
  void DecrementLinkCount(DWORD64 Inode);
  void DeleteInode(DWORD64 Inode);

  void AllocateExtentBuffer(S_LEAN_BLOCKS *extents, const unsigned count);
  void ReAllocateExtentBuffer(S_LEAN_BLOCKS *extents, const unsigned count);
  void FreeExtentBuffer(S_LEAN_BLOCKS *extents);
  
  CMyImageList m_TreeImages;
  HTREEITEM m_hRoot;
  BOOL      m_too_many;
  
  S_LEAN_SUPER m_super;
