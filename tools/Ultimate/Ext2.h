...
typedef struct _S_EXT2_SUPER {
  ...
  S_GUID uuid;        // 128-bit uuid for volume
  ...
  S_GUID journal_uuid;  // uuid of journal superblock
...
typedef struct _S_EXT2_GROUP_DESC {
  DWORD   block_bitmap;
  DWORD   inode_bitmap;
  DWORD   inode_table;
...
typedef struct _EXT3_EXTENT_HEADER {
...
typedef struct _S_EXT3_EXTENT_IDX {
...
typedef struct _S_EXT2_DIR_OLD {
  DWORD   inode;     // 1 based  (since a value of zero = not used)
  WORD    rec_len;
  WORD    name_len;
  // name goes here up to 255 chars
...
// Dialog Data
  ...
  void Ext2PutInode(const unsigned InodeNum, S_EXT2_INODE *inode);
  unsigned Ext2GetInode(const unsigned InodeNum, S_EXT2_INODE *inode);
  DWORD64 Ext2GetBlockNum(unsigned InodeNum, S_EXT2_INODE *inode, unsigned index);
  void GetInodeBlocks(unsigned InodeNum, S_EXT2_INODE *inode);
  
  void ParseDir(void *root, DWORD64 root_size, HTREEITEM parent);
  void *Ext2ReadFile(S_EXT2_INODE *inode, const unsigned InodeNum, const unsigned group);
  
  void SendToDialog(S_EXT2_SUPER *super);
  void ReceiveFromDialog(S_EXT2_SUPER *super);
