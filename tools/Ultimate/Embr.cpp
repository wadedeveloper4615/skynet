...
bool CEmbr::Exists(DWORD64 LBA) {
  ...
  if (m_exists) {
    ...
    S_EMBR_HDR *hdr = (S_EMBR_HDR *) m_entry_buffer;
    
    // check the CRC of the entries
    org_crc = hdr->crc;
    hdr->crc = 0;
    if (crc32(hdr, sizeof(S_EMBR_HDR) + (hdr->entry_count * sizeof(S_EMBR_ENTRY))) != org_crc) {
...
void CEmbr::UpdateEntry(CEmbrEntry *Entry, int index) {
  S_EMBR_ENTRY *entry = (S_EMBR_ENTRY *) ((BYTE *) m_entry_buffer + sizeof(S_EMBR_HDR) + (index * sizeof(S_EMBR_ENTRY)));
...
BOOL CEmbr::CheckEntry(int index) {
   S_EMBR_ENTRY *entries = (S_EMBR_ENTRY *) ((BYTE *) m_entry_buffer + sizeof(S_EMBR_HDR));
...
void CEmbr::OnCrcSet() {
  ...
  const unsigned size = sizeof(S_EMBR_HDR) + (entry_count * sizeof(S_EMBR_ENTRY));
  S_EMBR_HDR *hdr = (S_EMBR_HDR *) m_entry_buffer;
...
void CEmbr::OnUpdateCode() {
  ...
  memcpy(&code[0x1F2], sig, sizeof(S_EMBR_SIG));
...
void CEmbr::OnUpdateTotSects() {
  S_EMBR_ENTRY *entry = (S_EMBR_ENTRY *) ((BYTE *) m_entry_buffer + sizeof(S_EMBR_HDR));
  S_EMBR_HDR *hdr = (S_EMBR_HDR *) m_entry_buffer;
  int i;
