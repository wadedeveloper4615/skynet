...
void CFYSFS::OnSelchangedDirTree(NMHDR* pNMHDR, LRESULT* pResult) {
  ...
  S_FYSFS_ITEMS *items = (S_FYSFS_ITEMS *) m_dir_tree.GetDataStruct(hItem);
...
bool CFYSFS::FYSFSFormat(const BOOL AskForBoot, const BOOL clean) {
  ...
  if (m_hard_format) {
    ...
    memset(&m_super, 0, sizeof(S_FYSFS_SUPER));
  ...
  S_FYSOSSIG *s_sig = (S_FYSOSSIG *) (buffer + S_FYSOSSIG_OFFSET);
  ...
  m_super.crc = crc32(&m_super, sizeof(S_FYSFS_SUPER));
  ...
  S_FYSFS_ROOT *root = (S_FYSFS_ROOT *) buffer;
  ...
  root->crc = (BYTE) crc32(root, sizeof(S_FYSFS_ROOT));
...
void CFYSFS::Start(const DWORD64 lba, const DWORD64 size, const DWORD color, const int index, BOOL IsNewTab) {
  ...
  memcpy(&m_super, buffer, sizeof(S_FYSFS_SUPER));
  ...
  if (m_isvalid) {
    ...
    S_FYSFS_ROOT *root = (S_FYSFS_ROOT *) calloc((m_super.root_entries * sizeof(S_FYSFS_ROOT)) + (MAX_SECT_SIZE - 1), 1);
    if (root) {
      dlg->ReadFromFile(root, m_lba + m_super.root, ((m_super.root_entries * sizeof(S_FYSFS_ROOT)) + (dlg->m_sect_size - 1)) / dlg->m_sect_size);
...
void CFYSFS::ParseDir(S_FYSFS_ROOT *root, const unsigned entries, HTREEITEM parent, BOOL IsRoot) {
  S_FYSFS_ROOT *sub;
  HTREEITEM hItem;
  unsigned i = 0;
  CString name;
  ...
  while ((i<entries) && !m_too_many) {
    ...
    switch (root[i].sig) {
      ...
      case S_FYSFS_ROOT_SUB:
        ...
        sub = (S_FYSFS_ROOT *) ReadFile(&root[i]);
        if (sub) {
          ParseDir(sub, (unsigned) (filesize / sizeof(S_FYSFS_ROOT)), hItem, FALSE);
          free(sub);
        }
        break;
...
void CFYSFS::FysFSGetName(S_FYSFS_ROOT *root, unsigned index, CString &name, DWORD *attrb, DWORD64 *filesize) {
  unsigned i;
...
int CFYSFS::FillClusterList(S_FYSFAT_ENTRIES *EntryList, S_FYSFS_ROOT *root) {
  CUltimateDlg *dlg = (CUltimateDlg *) AfxGetApp()->m_pMainWnd;
  BYTE *buffer = (BYTE *) malloc(m_super.sect_clust * dlg->m_sect_size);
  S_FYSFS_ROOT *slot = (S_FYSFS_ROOT *) buffer;
  S_FYSFS_CONT *cont = (S_FYSFS_CONT *) buffer;
  //int i, j;
  int cnt = 0;
  //DWORD64 base;
...
void *CFYSFS::ReadFile(S_FYSFS_ROOT *root) {
  CUltimateDlg *dlg = (CUltimateDlg *) AfxGetApp()->m_pMainWnd;
  void *ptr = NULL;
  unsigned pos = 0, mem_size = 0;
  
  S_FYSFAT_ENTRIES ClusterList;
...
void CFYSFS::SaveItemInfo(HTREEITEM hItem, DWORD64 FileSize, S_FYSFS_ROOT *Entry, DWORD Sig, DWORD ErrorCode, BOOL CanCopy) {
  S_FYSFS_ITEMS *items = (S_FYSFS_ITEMS *) m_dir_tree.GetDataStruct(hItem);
  if (items) {
    ...
    items->Sig = Sig;
    if (Entry)
      memcpy(&items->Entry, Entry, sizeof(S_FYSFS_ROOT));
  }
}

// update all items in dialog
void CFYSFS::SendToDialog(S_FYSFS_SUPER *super) {
  CString cs;
...
void CFYSFS::ReceiveFromDialog(S_FYSFS_SUPER *super) {
