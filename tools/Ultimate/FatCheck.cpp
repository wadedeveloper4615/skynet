...
void CFat::OnFatCheck() {
  ...
  if (m_fat_size == FS_FAT32) {
    ...
    if (rootcluster > 0)  // make sure there is actually a root to read
      root = (S_FAT_ROOT *) ReadFile(rootcluster, &rootsize, TRUE);
  } else {
    rootcluster = 0;
    rootsize = bpb12->root_entrys * sizeof(S_FAT_ROOT);
    root = (S_FAT_ROOT *) ReadFile(rootcluster, &rootsize, TRUE);
  }
  if (root) {
    ...
    fcDirCount = 1;
    FatCheckRoot(modeless, root, rootsize / sizeof(S_FAT_ROOT), "\\");
    free(fcClusters);
    free(root);
  }
...
void CFat::FatCheckRoot(CModeless &modeless, S_FAT_ROOT *root, const unsigned entries, CString csPath) {
  S_FAT_ROOT *sub;
  unsigned cnt, i = 0, j;
