...
void CExt2::OnSelchangedDirTree(NMHDR* pNMHDR, LRESULT* pResult) {
  ...
  S_EXT2_ITEMS *items = (S_EXT2_ITEMS *) m_dir_tree.GetDataStruct(hItem);
...
void CExt2::Start(const DWORD64 lba, const DWORD64 size, const DWORD color, const int index, BOOL IsNewTab) {
  ...
  memcpy(&m_super, buffer, sizeof(S_EXT2_SUPER));
...
void CExt2::ParseDir(void *root, DWORD64 root_size, HTREEITEM parent) {
  S_EXT2_DIR *cur = (S_EXT2_DIR *) root, *sub;
  S_EXT2_INODE inode;
  HTREEITEM hItem;
  unsigned group;
  CString name;
  ...
  while (((BYTE *) cur < ((BYTE *) root + root_size)) && !m_too_many) {
      ...
      switch (file_type) {
        ...
        case EXT2_FT_DIR:    // File type: directory
          ...
          if (!IsDot) {
            ...
            sub = (S_EXT2_DIR *) Ext2ReadFile(&inode, cur->inode - 1, group);
            if (sub) {
    ...
    cur = (S_EXT2_DIR *) ((BYTE *) cur + cur->rec_len);
  }
}
...
void CExt2::GetGroupInodeTable(const int group) {
  ...
  desc = (S_EXT2_GROUP_DESC *) ((BYTE *) m_desc_table + (group * size));
...
void CExt2::WriteGroupInodeTable(const int group) {
  CUltimateDlg *dlg = (CUltimateDlg *) AfxGetApp()->m_pMainWnd;
  const int size = (m_super.feature_incompat & EXT4_FEATURE_INCOMPAT_64BIT) ? 64 : 32;
  S_EXT2_GROUP_DESC *desc = (S_EXT2_GROUP_DESC *) ((BYTE *) m_desc_table + (group * size));
...
void CExt2::GetInodeBlocks(unsigned InodeNum, S_EXT2_INODE *inode) {
  CUltimateDlg *dlg = (CUltimateDlg *) AfxGetApp()->m_pMainWnd;
  unsigned i;
  ...
  if (inode->flags & EXT4_EXTENTS_FL) {
    if (inode->extents.extent_hdr.depth == 0) {
      S_EXT3_EXTENT *extent = (S_EXT3_EXTENT *) inode->extents.extents;
...
DWORD64 CExt2::Ext2GetBlockNum(unsigned InodeNum, S_EXT2_INODE *inode, unsigned index) {
  GetInodeBlocks(InodeNum, inode);
  return m_block_table[index];
}

void *CExt2::Ext2ReadFile(S_EXT2_INODE *inode, const unsigned InodeNum, const unsigned group) {
  CUltimateDlg *dlg = (CUltimateDlg *) AfxGetApp()->m_pMainWnd;
...
void CExt2::Ext2PutInode(const unsigned InodeNum, S_EXT2_INODE *inode) {
...
unsigned CExt2::Ext2GetInode(const unsigned InodeNum, S_EXT2_INODE *inode) {
  const unsigned group = InodeNum / m_super.inodes_per_group;
  const unsigned in_num = (InodeNum - (group * m_super.inodes_per_group));
...
void CExt2::SendToDialog(S_EXT2_SUPER *super) {
...
void CExt2::ReceiveFromDialog(S_EXT2_SUPER *super) {
...
bool CExt2::Ext2Format(const BOOL AskForBoot) {
  ...
  */
  ...
  S_EXT2_SUPER *super = (S_EXT2_SUPER *) buffer;
  memset(super, 0, sizeof(S_EXT2_SUPER));
  ...
  for (i=0; i<groups; i++) {
    S_EXT2_GROUP_DESC *desc = (S_EXT2_GROUP_DESC *) ((BYTE *) desc_table_buffer + (i * group_desc_size));
...
S_ATTRIBUTES ext2_current_state[] = {
                                           //            |                               | <- max (col 67)
