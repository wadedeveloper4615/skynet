...
void CISOPrimary::OnSelchangedDirTree(NMHDR* pNMHDR, LRESULT* pResult) {
  ...
  S_ISO_ITEMS *items = (S_ISO_ITEMS *) m_dir_tree.GetDataStruct(hItem);
...
void CISOPrimary::Start(const DWORD64 lba, DWORD color, BOOL IsNewTab) {
  S_ISO_PVD *pvd = (S_ISO_PVD *) m_descriptor;
...
void CISOPrimary::DoRoot(void) {
  ...
  S_ISO_ROOT *root;
  root = (S_ISO_ROOT *) ReadFile(pvd->root.extent_loc, pvd->root.data_len, pvd->root.flags, TRUE);
  if (root) {
    SaveItemInfo(m_hRoot, &pvd->root, 0, FALSE);
...
DWORD CISOPrimary::CheckRootEntry(S_ISO_ROOT *r) {
  int i;
...
bool CISOPrimary::ParseDir(S_ISO_ROOT *root, DWORD datalen, HTREEITEM parent, BOOL IsRoot) {
  S_ISO_ROOT *r = root, *sub;
  HTREEITEM hItem;
  char szName[256+1];
  CString name;
  DWORD ErrorCode;
    ...
    if (ErrorCode != ISO_NO_ERROR) {
      ...
      if ((r->fi_len == 1) && (r->ident[0] == 0)) {
        ...
        if (r->flags & ISO_ROOT_FLAGS_DIR) {
          ...
          sub = (S_ISO_ROOT *) ReadFile(r->extent_loc, r->data_len, r->flags, FALSE);
          if (sub) {
...
void CISOPrimary::SaveItemInfo(HTREEITEM hItem, S_ISO_ROOT *root, DWORD ErrorCode, BOOL CanCopy) {
  S_ISO_ITEMS *items = (S_ISO_ITEMS *) m_dir_tree.GetDataStruct(hItem);
  if (items) {
...
void CISOPrimary::CopyFile(HTREEITEM hItem, CString csName) {
  S_ISO_ITEMS *items = (S_ISO_ITEMS *) m_dir_tree.GetDataStruct(hItem);
  if (items) {
    S_ISO_ROOT *root = (S_ISO_ROOT *) items->RootEntry;
