...
int CImageBar::DetectFileSystem(const DWORD64 lba, const DWORD64 size) {
  ...
  fs_type = DetectFat((S_FAT1216_BPB *) buffer, dlg->m_sect_size);
  if (fs_type > -1) {
    free(buffer);
    return fs_type;
  }
...
int CImageBar::DetectFat(S_FAT1216_BPB *bpb, const unsigned sect_size) {
 ...
 */
unsigned CImageBar::DetectFatSize(S_FAT1216_BPB *bpb) {
  ...
  const unsigned fat_size = (bpb->sect_per_fat != 0) ? bpb->sect_per_fat : ((S_FAT32_BPB *) bpb)->sect_per_fat32;
  ...
  if (memcmp((void *) bpb->sys_type, "FAT12   ", 8) == 0)
    ...
    fat16++;
  else if (memcmp(((S_FAT32_BPB *) bpb)->sys_type, "FAT32   ", 8) == 0)
    fat32++;
  ...
  if ((fat16 > 0) && (memcmp((void *) bpb->sys_type, "FAT16   ", 8) != 0))
    fat16--;
  if ((fat32 > 0) && (memcmp(((S_FAT32_BPB *) bpb)->sys_type, "FAT32   ", 8) != 0))
    fat32--;
...
int CImageBar::DetectExt2(void *buffer, const DWORD64 Size, const unsigned sect_size) {
  S_EXT2_SUPER *super = (S_EXT2_SUPER *) ((BYTE *) buffer + (2 * sect_size));
...
int CImageBar::DetectNTFS(void *buffer) {
  S_NTFS_BPB *bpb = (S_NTFS_BPB *) buffer;
...
int CImageBar::DetectSFS(void *buffer, const unsigned sect_size) {
  S_SFS_SUPER *super = (S_SFS_SUPER *) ((BYTE *) buffer + SFS_SUPER_LOC);
  BYTE crc = 0;
...
int CImageBar::DetectFYSFS(void *buffer, const unsigned sect_size) {
  S_FYSFS_SUPER *super = (S_FYSFS_SUPER *) ((BYTE *) buffer + (16 * sect_size));
