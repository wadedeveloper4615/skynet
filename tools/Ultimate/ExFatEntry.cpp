...
BOOL CExFatEntry::OnInitDialog() {
  ...
  if (LoadEntry()) {
    // fill dialog here
    root = (S_EXFAT_ROOT *) ((BYTE *) m_entry_buffer + m_root_offset);
...
void CExFatEntry::OnFlags() {
  S_EXFAT_ROOT *root = (S_EXFAT_ROOT *) ((BYTE *) m_entry_buffer + m_root_offset);
  CAttribute dlg;
  CString cs;
...
void CExFatEntry::OnCrcUpdate() {
  ...
  if (root[0].entry_type == EXFAT_DIR_ENTRY) {
    ...
    crc = m_Parent->ExFatCheckDirCRC((BYTE *) root, ((secondary_count + 1) * sizeof(S_EXFAT_ROOT)));
