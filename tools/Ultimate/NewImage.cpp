...
void CNewImage::OnCreateImage() {
  ...
  if (m_type == 3) {
    gpt_entries_size = ((((part_count > MAX_GPT_ENTRIES) ? MAX_GPT_ENTRIES : part_count) * sizeof(S_GPT_ENTRY)) + (m_sector_size - 1)) / m_sector_size;
  ...
  if ((m_options == 2) || (m_options == 3)) {  // Dynamic or Flat VDI file
    ...
    VDI_HEADER *header = (VDI_HEADER *) buffer;
  ...
  if (m_type <= 3) {
    ...
    if (hasMBR) {
      ...
      MBR_PART_ENTRY *part = (MBR_PART_ENTRY *) &buffer[512 - 2 - (4 * 16)];
      i = 0;
      if (hasEMBR) {
    ...
    if (hasEMBR) {
      ...
      S_EMBR_SIG *sig = (S_EMBR_SIG *) &buffer[0x1F2];
      sig->sig = EMBR_SIG_SIG;
      sig->offset = 20;
      sig->remaining = EMBR_RESVED_SECTS;
      sig->boot_sig = 0xAA55;
      
      S_EMBR_HDR *hdr = (S_EMBR_HDR *) (buffer + ((sig->offset - 1) * m_sector_size));
      ...
      S_EMBR_ENTRY *entries = (S_EMBR_ENTRY *) ((BYTE *) hdr + sizeof(S_EMBR_HDR));
      qword = 0;
      ...
      // calculate CRC
      hdr->crc = crc32(hdr, sizeof(S_EMBR_HDR) + (i * sizeof(S_EMBR_ENTRY)));
    ...
    if (hasGPT) {
      ...
      S_GPT_HDR *hdr = (S_GPT_HDR *) buffer;
      ...
      S_GPT_ENTRY *entries = (S_GPT_ENTRY *) (buffer + m_sector_size);
      ...
      hdr->crc32_entries = crc32(entries, (128 * sizeof(S_GPT_ENTRY))); //hdr->crc32_entries = crc32(entries, (i * sizeof(struct S_GPT_ENTRY)));
      // calculate CRC
  ...
  // Note:  We do not call the dlg->Write function when creating an ISO
    ...
    if (partition->m_sectors >= (32 + ((img_size + 3) / 4))) {
      ...
      // create the PVD
      S_ISO_PVD *pvd = (S_ISO_PVD *) buffer;
      pvd->type = 1;
      ...
      if (img_size > 0) {  // has boot image file
        ...
        S_ISO_BVD *bvd = (S_ISO_BVD *) buffer;
        bvd->type = 0;
        ...
        memset(buffer, 0, 2048);
        S_ISO_TERM *term = (S_ISO_TERM *) buffer;
        term->id = 255;
        ...
        memset(buffer, 0, 2048);
        S_ISO_BOOT_CAT *boot_cat = (S_ISO_BOOT_CAT *) buffer;
      ...
      S_ISO_PATH *path_tab = (S_ISO_PATH *) buffer;
      path_tab->len_di = 1;
      ...
      // root table
      memset(buffer, 0, 2048);
      S_ISO_ROOT *root = (S_ISO_ROOT *) buffer;
      ...
      memcpy(&root[1], &root[0], sizeof(S_ISO_ROOT));
      root[1].ident[0] = 1;
